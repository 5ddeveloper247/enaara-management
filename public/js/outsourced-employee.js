(function () {
    ...
    function exportOutsourcedEmployeesToCsv(rows) {
        const headers = [
            'Full Name',
            'CNIC Number',
            'Mobile Number',
        ...
        rows.forEach(function (r) {
            lines.push([
                ...
                csvEscape(normalizeValue(r.cnic_number)),
                csvEscape(normalizeValue(r.mobile_number)),
                csvEscape(normalizeValue(r.contractor_company_name)),
                csvEscape(normalizeValue(r.supervisor_name)),
                csvEscape(normalizeValue(r.supervisor_contact_number)),
                csvEscape(normalizeValue(r.organization)),
                csvEscape(normalizeValue(r.sbu)),
                csvEscape(normalizeValue(r.department)),
                csvEscape(normalizeValue(r.job_role_trade)),
                csvEscape(normalizeValue(r.placement_floor)),
                csvEscape(normalizeValue(r.date_of_deployment)),
                csvEscape(normalizeValue(r.biometric_id)),
                csvEscape(r.attendance_access ? 'Granted' : 'Not Granted')
            ].join(','));
        });
        ...
        a.download = 'outsourced_employees_' + new Date().toISOString().slice(0, 10) + '.csv';
        ...
        if (exportBtn) {
            $('#exportBtn').off('click').on('click', function () {
                if (currentMode === 'outsourced') {
                    ...
                    $.get(window.outsourcedEmployeeDataUrl, params, function (res) {
                        if (!res || !res.success || !res.data || !res.data.length) {
                            alert('No outsourced employee data to export.');
                            return;
                        }
                        exportOutsourcedEmployeesToCsv(res.data);
                        if (window.Swal) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Export Successful',
                                text: 'Outsourced employee records have been exported successfully.',
                                confirmButtonColor: '#012445'
                            });
                        } else {
                            alert('Outsourced employee records have been exported successfully.');
                        }
                    }).fail(function () {
                        if (window.Swal) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Export Failed',
                                text: 'Could not export outsourced employees. Please try again.',
                                confirmButtonColor: '#dc3545'
                            });
                        } else {
                            alert('Could not export outsourced employees. Please try again.');
                        }
                    });
                    return;
                }
