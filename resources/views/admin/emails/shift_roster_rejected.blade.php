...
<head>
  ...
  <title>Shift Roster Rejected – Enaara HRMS</title>
  <style>
    ...
    .highlight { background: #fdf4f4; border-left: 3px solid #dc3545; padding: 14px 18px; border-radius: 0 6px 6px 0; margin: 20px 0; }
...
<body>
  <div class="wrapper">
    ...
    <div class="body">
      <h2>Shift roster rejected</h2>
      <p>Hi <strong>{{ $recipientName }}</strong>,</p>
      <p>Your submitted shift roster was rejected and has not been applied to the shift planner.</p>
      <div class="highlight">
        <strong>Rejected Roster</strong>
        <div class="info-row"><span class="label">Employee</span><span>{{ $assigneeName }}</span></div>
        <div class="info-row"><span class="label">Rejected By</span><span>{{ $approverName }}</span></div>
        <div class="info-row"><span class="label">Shift</span><span>{{ $shiftLabel }}</span></div>
        <div class="info-row"><span class="label">Period</span><span>{{ $startDate }} – {{ $endDate }}</span></div>
        <div class="info-row"><span class="label">Shift Days</span><span>{{ $shiftCount }}</span></div>
        <div class="info-row"><span class="label">Off Days</span><span>{{ $offDayCount }}</span></div>
        @if($rejectionReason !== '')
        <div class="info-row"><span class="label">Reason</span><span>{{ $rejectionReason }}</span></div>
        @endif
      </div>
      <p>Please review the roster details and submit a revised request if needed.</p>
      <div class="btn-wrap">
        <a href="{{ $actionUrl }}" class="btn">Open Shift Planner</a>
      </div>
    </div>
    <div class="footer">
