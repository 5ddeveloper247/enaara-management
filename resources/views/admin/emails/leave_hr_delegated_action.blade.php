...
<head>
  ...
  <title>Leave Action on Your Behalf – Enaara HRMS</title>
  <style>
...
<body>
  <div class="wrapper">
    ...
    <div class="body">
      <h2>Leave request action taken on your behalf</h2>
      <p>Hi <strong>{{ $recipientName }}</strong>,</p>
      <p>
        <strong>{{ $hrActorName }}</strong> from the Human Resource department has taken action on a leave request
        that was assigned to you as the approver.
      </p>

      <div class="highlight">
        <strong>Leave Details</strong>
        <div class="info-row"><span class="label">Employee</span><span>{{ $applicantName }}</span></div>
        ...
        <div class="info-row"><span class="label">Action Taken</span><span style="font-weight: 700;">{{ $statusLabel }}</span></div>
        <div class="info-row"><span class="label">Action By</span><span>{{ $hrActorName }} (HR)</span></div>
      </div>

      <p>Please review the updated leave request in Enaara HRMS if further action is required from your side.</p>
      <div class="btn-wrap">
        <a href="{{ $actionUrl }}" class="btn">View Leave Requests</a>
      </div>
    </div>
    <div class="footer">
