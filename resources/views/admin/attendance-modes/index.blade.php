...
@section('title', 'Attendance Modes - Admin Panel')

@section('page-title', 'Attendance Modes')
    ...
    <div class="container-fluid">
        <div class="card border-0 rounded-4 mb-4">
            <div class="card-body p-0">
                @include('admin.attendance-modes.header')
                @include('admin.attendance-modes.counters')
                @include('admin.attendance-modes.attendance_modes_table')
            </div>
        </div>
    </div>
    @include('admin.attendance-modes.delete-modal')
    @include('admin.attendance-modes.detail_canvas')
@endsection
...
@push('scripts')
    ...
    <script>
        let attendanceModesTable;
        let itemToDelete = null;
        const csrfToken = document.querySelector('meta[name="csrf-token"]') ? document.querySelector('meta[name="csrf-token"]').getAttribute('content') : '';
        const attendanceModeStatusUrl = "{{ url('/admin/attendance-modes') }}";
        const attendanceModeDestroyUrl = "{{ url('/admin/attendance-modes') }}";

        $(document).ready(function() {
            attendanceModesTable = $('#attendanceModesTable').DataTable({
                order: [[0, 'desc']],
                pageLength: 10,
                responsive: true,
            ...
            $(document).on('click', '.view-attendance-mode', function() {
                var btn = $(this);
            ...
            $(document).on('change', '.status-toggle', function() {
                var checkbox = $(this);
                var id = checkbox.data('attendance-mode-id');
                var isActive = checkbox.is(':checked');
                var row = checkbox.closest('tr');
                $.ajax({
                    url: attendanceModeStatusUrl + '/' + id + '/status',
                    method: 'PATCH',
            ...
            const deleteModal = document.getElementById('deleteAttendanceModeModal');
            if (deleteModal) {
                deleteModal.addEventListener('show.bs.modal', function(event) {
                    const button = event.relatedTarget;
                    if (!button) return;
                    const name = button.getAttribute('data-attendance-mode-name');
                    $('#deleteAttendanceModeName').text(name || '');
                    itemToDelete = button.getAttribute('data-attendance-mode-id');
                });
            }

            $('#confirmDeleteAttendanceModeBtn').on('click', function() {
                if (itemToDelete === null) return;
                var id = itemToDelete;
                itemToDelete = null;
                $.ajax({
                    url: attendanceModeDestroyUrl + '/' + id,
                    method: 'DELETE',
                    headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                    success: function() {
                        attendanceModesTable.rows().every(function() {
                            if ($(this.node()).data('attendance-mode-id') == id) {
                                attendanceModesTable.row(this).remove().draw();
                                return false;
                            }
                        });
                        var total = parseInt($('#totalAttendanceModes').text(), 10) || 0;
                        $('#totalAttendanceModes').text(Math.max(0, total - 1));
                        var modalEl = document.getElementById('deleteAttendanceModeModal');
            ...
            $('#applyFiltersBtn').on('click', function() {
                ...
                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                    var row = attendanceModesTable.row(dataIndex).node();
