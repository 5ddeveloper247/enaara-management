            ...
            @forelse($sbus ?? [] as $sbu)
                <div class="col-md-6 col-lg-4">
                    <div class="card sbu-card border-1 rounded-3 h-100"
                        data-sbu-status="{{ $sbu->is_active ? 'active' : 'inactive' }}">
                        <div class="card-body p-4">
                            ...
                            <div class="mt-3 pt-3 border-top d-flex gap-1">
                                ...
                                <button type="button" class="btn btn-sm btn-outline-secondary view-sbu-btn"
                                    data-bs-toggle="offcanvas" data-bs-target="#sbuDetailCanvas"
                                    data-sbu-id="{{ $sbu->id }}" data-sbu-name="{{ e($sbu->name) }}"
                ...
                <div class="mb-4">
                    ...
                    <div class="bg-transparent">
                        <label class="list-group-item list-group-item-action border-0 px-0 py-2 cursor-pointer">
                            <input class="form-check-input me-2 filter-status" type="radio" name="filterStatus"
                                value="all" id="filterStatusAll" checked>
                            All
                        </label>
                        <label class="list-group-item list-group-item-action border-0 px-0 py-2 cursor-pointer">
                            <input class="form-check-input me-2 filter-status" type="radio" name="filterStatus"
                                value="active" id="filterStatusActive">
                            Active
                        </label>
                        <label class="list-group-item list-group-item-action border-0 px-0 py-2 cursor-pointer">
                            <input class="form-check-input me-2 filter-status" type="radio" name="filterStatus"
                                value="inactive" id="filterStatusInactive">
                            Inactive
                        </label>
                    </div>
                </div>
