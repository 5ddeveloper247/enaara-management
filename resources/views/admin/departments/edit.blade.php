...
@section('title', 'Edit Department - Admin Panel')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="organization_id" id="organization_id" class="form-select @error('organization_id') is-invalid @enderror" required>
                                ...
                                @foreach($organizations ?? [] as $org)
                                    <option value="{{ $org->id }}" {{ old('organization_id', $department->organization_id) == $org->id ? 'selected' : '' }}>{{ $org->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="sbu_id" id="sbu_id" class="form-select @error('sbu_id') is-invalid @enderror" required>
                                ...
                                @foreach($sbus ?? [] as $sbu)
                                    <option value="{{ $sbu->id }}" {{ old('sbu_id', $department->sbu_id) == $sbu->id ? 'selected' : '' }}>{{ $sbu->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $department->name) }}" required maxlength="255">
                            @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label for="code" class="form-label">Code</label>
                            <input type="text" name="code" id="code" class="form-control @error('code') is-invalid @enderror" value="{{ old('code', $department->code) }}" maxlength="64">
                            ...
                            <select name="parent_department_id" id="parent_department_id" class="form-select @error('parent_department_id') is-invalid @enderror">
                                ...
                                @foreach($parentDepartments ?? [] as $parent)
                                    @if($parent->id != $department->id)
                                        <option value="{{ $parent->id }}" {{ old('parent_department_id', $department->parent_department_id) == $parent->id ? 'selected' : '' }}>{{ $parent->name }} {{ $parent->organization ? '(' . $parent->organization->name . ')' : '' }}</option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            <button type="submit" class="btn btn-primary bg-main border-0">Update Department</button>
